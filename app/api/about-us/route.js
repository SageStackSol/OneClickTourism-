import { connectDB } from "@/lib/mongodb";
import cloudinary from "@/lib/cloudinary";
import AboutUs from "@/models/AboutUs";
import { NextResponse } from "next/server";

// 📍 GET
export async function GET() {
  try {
    await connectDB();
    const aboutUs = await AboutUs.find().sort({ createdAt: -1 });
    return new Response(JSON.stringify(aboutUs), { status: 200 });
  } catch (error) {
    return new Response(JSON.stringify({ message: "Error fetching aboutUs", error: error.message }), { status: 500 });
  }
}

//post 
export const POST = async (req) => {
  try {
    const { aboutUsHeroImages} = await req.json();

    // if (!heading || !tagline) {
    //   return NextResponse.json({ error: "Heading and tagline required" }, { status: 400 });
    // }

    await connectDB();

    // Upload hero section images
    const heroUrls = await Promise.all(
      (aboutUsHeroImages || []).map(async (base64) => {
        const uploaded = await cloudinary.uploader.upload(base64, {
          folder: "about_hero_section",
        });
        return uploaded.secure_url;
      })
    )
   

    // Save to MongoDB
    const aboutUs = await AboutUs.create({
    
      aboutUsHeroImages: heroUrls,

    });

    return NextResponse.json({
      message: "Data saved successfully",
      data: aboutUs,
    });
  } catch (error) {
    console.error("Upload error:", error);
    return NextResponse.json({ error: "Upload failed" }, { status: 500 });
  }
};