...
function EnquiryForm() {
  ...
  const [destinationDetailsFetched, setDestinationDetailsFetched] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (service === "Inbound") {
      ...
      const fetchInbound = async () => {
        try {
          const res = await fetch(`/api/inbound/${inboundDestination}`);
          const data = await res.json();
          setFormData((prev) => ({
            ...prev,
            inbound: { ...prev.inbound, destination: data.name || "" },
          }));
          setDestinationDetailsFetched(true);
          if (!res.ok) throw new Error(data.message);
setDestinationImage(data.image)
    ...
    if (service === "Destinations") {
      ...
      const fetchDestination = async () => {
        try {
          const res = await fetch(`/api/destination/${destination}`);
          const data = await res.json();
          setFormData((prev) => ({
            ...prev,
            destination: {
              ...prev.destination,
              preferredDestination: data.name || "",
            },
          }));
          setDestinationDetailsFetched(true);
        ...
        <div>
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-12 mt-8 lg:gap-12 md:gap-12 gap-6">
              ...
              {/* RIGHT: Services */}
              <div className="lg:col-span-6 md:col-span-6 col-span-12 flex flex-col items-center ">

  <img src={destinationImage} className="h-[50%] rounded mb-4"/>
  <p className="font-semibold">{destinationDescription}</p>
